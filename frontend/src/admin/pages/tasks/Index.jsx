...
const AdminTaskIndex = () => {
    ...
    const fetchTasks = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`import.meta.env.VITE_API_URL/admin/tasks?page=${currentPage}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem("adminToken")}` },
            });
            if (res.data.status) {
                // Laravel pagination struct might be res.data.data or res.data.data.data depending on controller
                // Checking previous controllers, usually it's res.data.data.data for paginated
    ...
    return (
        <AdminLayout>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Manajemen Tugas Writer</h1>
